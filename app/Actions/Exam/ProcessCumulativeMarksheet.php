...
class ProcessCumulativeMarksheet
{
    ...
    public function execute(Batch $batch, Collection $students, array $params)
    {
        ...
        $schedules = Schedule::query()
            ...
            ->whereHas('exam', function ($q) use ($terms) {
                $q->whereIn('term_id', $terms->pluck('id')->all());
            })
            ->where(function ($q) {
        ...
        foreach ($terms as $term) {
            $colspan = 0;
            foreach ($schedules as $schedule) {
                if ($schedule->exam->term_id == $term->id) {
                    $colspan++;
                }
            }
        ...
        foreach ($terms as $term) {
            $termSchedules = $schedules->where('exam.term_id', $term->id);

            foreach ($termSchedules as $schedule) {
                array_push($row, [
                    'key' => 'exam',
                    'type' => 'heading',
                    'label' => $schedule->exam->display_name ?? $schedule->exam->name,
                    'colspan' => 1,
                ]);
            }
        ...
        foreach ($students as $student) {
            ...
            foreach ($terms as $term) {
                $termTotal[$term->id]['obtained_marks'] = 0;
                $termTotal[$term->id]['max_marks'] = 0;
            }
            ...
            foreach ($studentPrimarySubjects as $subject) {
                ...
                foreach ($terms as $term) {
                    ...
                    $termTotal[$term->id]['obtained_marks'] += $subjectWiseTotal;
                    $termTotal[$term->id]['max_marks'] += $subjectWiseMaxMarks;
                }
            ...
            foreach ($terms as $term) {
                ...
                $termObtainedMarks = $termTotal[$term->id]['obtained_marks'];
                $termMaxMarks = $termTotal[$term->id]['max_marks'];

                $termPercentage = $termMaxMarks ? round(($termObtainedMarks / $termMaxMarks) * 100, 2) : 0;

                $termGrade = $this->getGrade($lastSchedule->grade, $termMaxMarks, $termObtainedMarks, 'code');

                array_push($termWiseRow, [
                    'key' => 'total',
                    'type' => 'footer',
                    'label' => $termObtainedMarks,
                ]);

                array_push($termWiseRow, [
                    'key' => 'percentage',
                    'type' => 'footer',
                    'label' => $termPercentage,
                ]);
                ...
                array_push($termTotalRow, [
                    'key' => 'total',
                    'type' => 'footer',
                    'colspan' => $schedules->where('exam.term_id', $term->id)->count() + 3,
                    'label' => $termObtainedMarks.' / '.$termMaxMarks,
                ]);
