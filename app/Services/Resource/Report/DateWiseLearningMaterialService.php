...
class DateWiseLearningMaterialService
{
    ...
    public function generate(Request $request)
    {
        ...
        $learningMaterials = LearningMaterial::query()
            ->byPeriod()
            ->with('records')
            ->whereDate('published_at', $request->query('date', today()->toDateString()))
            ->get();
        ...
        foreach ($batches as $batch) {
            ...
            foreach ($batchSubjectRecords as $subjectRecord) {
                ...
                $subjectLearningMaterials = $learningMaterials->filter(function ($learningMaterial) use ($subject, $batch) {
                    return $learningMaterial->records->contains(function ($record) use ($subject, $batch) {
                        return $record->subject_id === $subject->id && $record->batch_id === $batch->id;
                    });
                });

                $submittedEmployees = [];
                foreach ($subjectLearningMaterials as $learningMaterial) {
                    $submittedEmployees[] = [
                        'name' => $employees->firstWhere('id', $learningMaterial->employee_id)?->contact?->name,
                        'created_at' => \Cal::dateTime($learningMaterial->created_at)?->formatted,
                    ];
                }
        ...
        if ($request->query('output') == 'pdf') {
            $content = view()->first([config('config.print.custom_path').'resource.report.date-wise-learning-material', 'print.resource.report.date-wise-learning-material'], compact('data', 'layout', 'date'))->render();
        ...
        return view()->first([config('config.print.custom_path').'resource.report.date-wise-learning-material', 'print.resource.report.date-wise-learning-material'], compact('data', 'layout', 'date'))->render();
    }
}
