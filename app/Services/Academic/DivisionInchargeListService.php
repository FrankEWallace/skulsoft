...
class DivisionInchargeListService extends ListGenerator
{
    ...
    public function filter(Request $request): Builder
    {
        ...
        return Incharge::query()
            ->whereHasMorph(
                'model',
                [Division::class],
                function (Builder $query) use ($divisions) {
                    $query->byPeriod()
    ...
    {
        return DivisionInchargeResource::collection($this->filter($request)
