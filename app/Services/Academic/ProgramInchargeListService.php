...
class ProgramInchargeListService extends ListGenerator
{
    ...
    {
        $programs = Str::toArray($request->query('programs'));
        ...
        return Incharge::query()
            ->whereHasMorph(
                'model',
                [Program::class],
                function (Builder $query) use ($programs) {
                    $query->byTeam()
                        ->when($programs, function ($q, $programs) {
                            $q->whereIn('uuid', $programs);
                        });
                }
            )
    ...
    {
        return ProgramInchargeResource::collection($this->filter($request)
